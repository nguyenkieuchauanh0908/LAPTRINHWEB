...
public class employeeListController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		List<User> employeeList = userDAO.getAllUserByPage(indexpage,n,role); // lấy các user/nhân viên thuộc trang thứ indexPage
																			// cần hiển thị
		// B3:thiết lập dữ liệu trên jsp
		req.setAttribute("employeeList", employeeList);
		req.setAttribute("CountPa", countPage);
		req.setAttribute("tag", indexpage);
		RequestDispatcher rq = req.getRequestDispatcher("views/admin/Employee/employeeList.jsp");
		rq.forward(req, resp);
	}
